import { POST } from "@/app/api/travels/[id]/tasks/route";
...
describe("POST /travels/[id]/tasks", () => {
    beforeEach(() => jest.clearAllMocks());

    it("✅ crée la tâche si session valide et participant", async () => {
        (getServerSession as jest.Mock).mockResolvedValue(mockSession);
        prismaMock.trip.findUnique.mockResolvedValue({
            id: "t1",
            participants: [{ userId: "u1" }],
        });
        prismaMock.task.create.mockResolvedValue({ id: "task1", title: "Todo" });

        const res = (await runPost({ id: "t1" }, { title: "Todo" })) as NextResponse;
        ...
        expect(prismaMock.task.create).toHaveBeenCalledWith({
            data: { title: "Todo", description: undefined, tripId: "t1" },
        });
    ...
    it("❌ retourne 400 si données invalides", async () => {
        ...
        const res = (await runPost({ id: "t1" }, { title: "" })) as NextResponse;
